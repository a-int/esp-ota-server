...
static void connect_handler(void* arg, esp_event_base_t event_type, int32_t event_id, void* data){
    reconnect_try_counter = 0;
    ip_event_got_ip_t* ip = (ip_event_got_ip_t*)data;
    ESP_LOGI(TAG, "Got IP: " IPSTR, IP2STR(&ip->ip_info.ip));
}

static void disconnect_handler(void* arg, esp_event_base_t event_type, int32_t event_id, void* data){
